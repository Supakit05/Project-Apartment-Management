...
import { getRooms, getMaintenanceTasks, saveMaintenanceTask, getUserMaintenanceTasks } from '../../services/api';
...
export const ResidentMaintenancePage: React.FC = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    description: '',
    preferredTime: 'ช่วงเช้า (09:00 - 12:00 น.)',
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // If empty or if user is admin, also fetch from main list
      if (allUserTasks.length === 0) {
        const allTasks = await getMaintenanceTasks();
        if (user) {
          allUserTasks = allTasks.filter(t => 
            (t.reporterEmail && t.reporterEmail.toLowerCase() === user.email.toLowerCase()) ||
            (t.reporterName && t.reporterName.toLowerCase() === user.fullname.toLowerCase()) ||
            (user.phone && t.reporterPhone === user.phone) ||
            user.role === 'admin'
          );
          if (allUserTasks.length === 0 && user.role === 'admin') {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!isAuthenticated) {
      toast.info('กรุณาเข้าสู่ระบบก่อนส่งคำขอแจ้งซ่อมบำรุง', { duration: 5000 });
      navigate('/login');
      return;
    }

    if (!formData.description.trim()) {
      toast.error('กรุณาระบุรายละเอียดอาการชำรุด');
      return;
    }
    ...
    try {
      ...
      const payload: Partial<MaintenanceTask> = {
        ...
        status: 'Pending',
        assignedWorker: 'รอเจ้าหน้าที่มอบหมายช่าง',
      };

      const result = await saveMaintenanceTask(payload);
      toast.success(`ส่งคำขอแจ้งซ่อมห้อง ${formData.roomNumber} เรียบร้อยแล้ว! รหัสงาน: ${result.taskNo}`);
      ...
      toast.error('ไม่สามารถส่งคำขอแจ้งซ่อมได้ โปรดลองอีกครั้ง');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const getPriorityBadge = (p: string) => {
    switch (p) {
      case 'High':
        return <span className="px-2.5 py-1 rounded-full text-[10px] font-extrabold bg-rose-100 text-rose-800 dark:bg-rose-950/50 dark:text-rose-300">ด่วนมาก (High)</span>;
      case 'Medium':
        return <span className="px-2.5 py-1 rounded-full text-[10px] font-extrabold bg-amber-100 text-amber-800 dark:bg-amber-950/50 dark:text-amber-300">ปานกลาง (Medium)</span>;
      default:
        return <span className="px-2.5 py-1 rounded-full text-[10px] font-extrabold bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300">ปกติ (Low)</span>;
    }
  };
      ...
      case 'Completed':
        return (
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-emerald-100 text-emerald-800 dark:bg-emerald-950/50 dark:text-emerald-300">
            <CheckCircle2 className="w-3.5 h-3.5" /> ซ่อมเสร็จสิ้น (Completed)
          </span>
        );
      case 'In Progress':
        return (
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-blue-100 text-blue-800 dark:bg-blue-950/50 dark:text-blue-300">
            <Clock className="w-3.5 h-3.5 animate-spin" /> กำลังดำเนินการ (In Progress)
          </span>
        );
      case 'Pending':
      default:
    ...
    <div className="max-w-[1000px] mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      ...
      <div className="flex justify-center">
        <div className="inline-flex p-1.5 rounded-2xl bg-slate-100 dark:bg-nike-dark-elevated border border-slate-200 dark:border-slate-800">
          ...
          >
            <Plus className="w-4 h-4" /> แบบฟอร์มแจ้งซ่อมใหม่
          </button>
          <button
          ...
          >
            <Clock className="w-4 h-4" /> ประวัติและสถานะงานซ่อม ({tasks.length})
          </button>
        </div>
      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              <div>
                <label className="block text-xs font-bold text-nike-ink dark:text-white mb-2">
                  หมายเลขห้องพักที่ต้องการแจ้งซ่อม *
                </label>
                <select
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white text-xs font-bold focus:outline-none focus:ring-2 focus:ring-rose-500 transition-all cursor-pointer"
                >
                  <option value="Light bulb replacement">ระบบไฟฟ้า / เปลี่ยนหลอดไฟ (Electrical & Lighting)</option>
                  <option value="Air-con servicing">เครื่องปรับอากาศ / แอร์ไม่เย็น (Air-Conditioning)</option>
                  <option value="Plumbing">ระบบประปา / ก๊อกน้ำ / ท่อระบายน้ำ (Plumbing & Water)</option>
                  <option value="General Repair">เฟอร์นิเจอร์ / ประตูหน้าต่าง / ซ่อมทั่วไป (General Repair)</option>
                </select>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              <div>
                ...
                >
                  <option value="Low">ปกติ (Low) - ภายใน 2-3 วัน</option>
                  <option value="Medium">ปานกลาง (Medium) - ภายใน 24 ชม.</option>
                  <option value="High">ด่วนมาก (High) - น้ำรั่ว / ไฟฟ้าดับ / เร่งด่วน</option>
                </select>
              </div>

              <div>
                <label className="block text-xs font-bold text-nike-ink dark:text-white mb-2">
                  ช่วงเวลาที่สะดวกให้ช่างเข้าตรวจสอบ
                </label>
                <select
                ...
                >
                  ...
                  <option value="สะดวกทุกเวลา (เจ้าหน้าที่ติดต่อก่อนเข้า)">สะดวกทุกเวลา (เจ้าหน้าที่ติดต่อก่อนเข้า)</option>
                </select>
              </div>
            ...
            <div>
              <label className="block text-xs font-bold text-nike-ink dark:text-white mb-2">
                รายละเอียดอาการชำรุด / ปัญหาที่พบ *
              </label>
              <textarea
                rows={4}
                value={formData.description}
                onChange={(e) => setFormData({ ...formData, description: e.target.value })}
                placeholder="ระบุอาการชำรุด เช่น แอร์มีน้ำหยด, หลอดไฟห้องน้ำไม่ติด, ก๊อกน้ำอ่างล้างหน้าปิดไม่สนิท..."
            ...
            <div className="p-5 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 space-y-4">
              <span className="text-xs font-bold text-slate-700 dark:text-slate-300 block">
                ข้อมูลผู้แจ้ง / เบอร์โทรศัพท์ติดต่อ
              </span>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-[11px] font-medium text-slate-500 mb-1">ชื่อ-นามสกุล</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-[11px] font-medium text-slate-500 mb-1">เบอร์โทรศัพท์ติดต่อ *</label>
                  <input
                    type="tel"
        ...
        <div className="space-y-4">
          {loading ? (
            <div className="text-center py-16 text-slate-400 font-medium flex items-center justify-center gap-2">
              <RefreshCw className="w-4 h-4 animate-spin" /> กำลังโหลดประวัติงานซ่อม...
            </div>
          ) : tasks.length === 0 ? (
            <div className="text-center py-16 bg-white dark:bg-nike-dark-elevated rounded-3xl border border-nike-hairline dark:border-nike-dark-card space-y-3 p-8">
              ...
              <h3 className="text-base font-bold text-nike-ink dark:text-white">ยังไม่มีประวัติการแจ้งซ่อม</h3>
              <p className="text-xs text-nike-mute max-w-sm mx-auto">
                หากอุปกรณ์ในห้องพักชำรุด สามารถกดแท็บ "แบบฟอร์มแจ้งซ่อมใหม่" เพื่อส่งเรื่องให้แอดมินและช่างได้ทันที
              </p>
            </div>
          ) : (
            tasks.map(task => (
              ...
              >
                ...
                <div className="py-2">
                  <div className="grid grid-cols-3 gap-2 text-center text-xs font-medium">
                    <div className="space-y-1">
                      ...
                      <span className="text-nike-ink dark:text-white font-bold block">แจ้งเรื่องเข้าระบบ</span>
                      <span className="text-[10px] text-nike-mute block">{task.reportedDate || 'บันทึกแล้ว'}</span>
                    </div>
                    ...
                    <div className="space-y-1">
                      ...
                      <span className={`block font-bold ${task.status === 'In Progress' ? 'text-blue-600 font-extrabold' : 'text-slate-700 dark:text-slate-300'}`}>
                        เจ้าหน้าที่มอบหมายช่าง
                      </span>
                      <span className="text-[10px] text-nike-mute block">
                        {task.assignedWorker || 'รอประสานงานช่าง'}
                      </span>
                    </div>
                    ...
                    <div className="space-y-1">
                      ...
                      <span className="text-[10px] text-nike-mute block">
                        {task.status === 'Completed' ? 'เรียบร้อย' : 'รอช่างเข้าแก้ไข'}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-700/60 text-xs space-y-2">
                  <div>
                    <span className="text-slate-400 font-medium block">รายละเอียดอาการชำรุด:</span>
                  ...
                  {task.preferredTime && (
                    <div className="pt-2 border-t border-slate-200 dark:border-slate-700 flex items-center gap-2 text-slate-500 dark:text-slate-400">
                      ...
                      <span>ช่วงเวลาที่สะดวก: <strong className="text-slate-800 dark:text-white">{task.preferredTime}</strong></span>
                    </div>
                  )}
                  {task.assignedWorker && task.assignedWorker !== 'รอเจ้าหน้าที่มอบหมายช่าง' && task.assignedWorker !== 'รอแอดมินมอบหมายช่าง' && (
                    <div className="flex items-center gap-2 text-blue-600 dark:text-blue-400 font-semibold">
                      <User className="w-3.5 h-3.5" />
                      <span>ช่างผู้รับผิดชอบงาน: <strong>{task.assignedWorker}</strong></span>
                    </div>
                  )}
                </div>
